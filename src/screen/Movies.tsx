...
const Movies: React.FC = () => {
  ...
  const list = ['Popular TV', 'Top Rated TV'];
  ...
  const fetchDataMovies = (): void => {
    if (listNumber == 0) {
      ...
      setLoading(true);
      axios
        .get<IMoviesData[]>(topRatedMoviesUrl + token)
        .then((response: AxiosResponse) =>
          dispatch(SET_GET_TV_POPULAR(response.data.results)),
        )
        .then(() => setLoading(false))
  ...
  const handleRefetch = (index: number) => {
    setListNumber(index);
    fetchDataMovies();
  };
  ...
  return (
    <View style={styles.container}>
      <View style={{backgroundColor: 'grey', height: 70, marginTop: 80}}>
        {list.map((i, k) => {
          return (
            <TouchableOpacity
              style={{paddingBottom: 10}}
              key={k}
              onPress={() => handleRefetch(k)}
              disabled={k == listNumber}>
              <Text
                style={[
                  globalStyle.headerTitle,
                  {color: listNumber == k ? 'white' : 'black'},
                ]}>
                {i}
              </Text>
            </TouchableOpacity>
